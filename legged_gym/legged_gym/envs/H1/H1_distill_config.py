...
from legged_gym.envs.H1.H1_field_config import H1FieldCfg, H1FieldCfgPPO, H1RoughCfgPPO

multi_process_ = False
class H1DistillCfg( H1FieldCfg ):
    

    class env( H1FieldCfg.env ):
        num_envs = 256
        obs_components = [
            "lin_vel",
            "ang_vel",
    ...
    class terrain( H1FieldCfg.terrain ):
        if multi_process_:
            num_rows = 4
            num_cols = 1
        else:
        ...
        BarrierTrack_kwargs = merge_dict(H1FieldCfg.terrain.BarrierTrack_kwargs, dict(
            leap= dict(
                length= [0.05, 0.8],
                depth= [0.5, 0.8],
    ...
    class sensor( H1FieldCfg.sensor ):
    ...
    class commands( H1FieldCfg.commands ):
        ...
        lin_cmd_cutoff = 0.2
        class ranges( H1FieldCfg.commands.ranges ):
            # lin_vel_x = [0.6, 1.8]
            lin_vel_x = [-0.6, 2.0]
    ...
    class normalization( H1FieldCfg.normalization ):
        class obs_scales( H1FieldCfg.normalization.obs_scales ):
            forward_depth = 1.0

    class noise( H1FieldCfg.noise ):
        add_noise = False
        class noise_scales( H1FieldCfg.noise.noise_scales ):
            forward_depth = 0.0
    ...
    class curriculum( H1FieldCfg.curriculum ):
        no_moveup_when_fall = False

    class sim( H1FieldCfg.sim ):
        no_camera = False
    
logs_root = osp.join(osp.dirname(osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))), "logs")
class H1DistillCfgPPO( H1FieldCfgPPO ):
    class algorithm( H1FieldCfgPPO.algorithm ):
        entropy_coef = 0.0
        using_ppo = False
        ...
        # class teacher_policy( H1FieldCfgPPO.policy ):
    ...
    class policy( H1RoughCfgPPO.policy ):
    ...
    class runner( H1FieldCfgPPO.runner ):
        ...
        run_name = "".join(["H1_",
            ("{:d}skills".format(len(H1DistillCfg.terrain.BarrierTrack_kwargs["options"]))),

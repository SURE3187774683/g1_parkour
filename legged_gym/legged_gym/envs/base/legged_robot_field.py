...
class LeggedRobotFieldMixin:
    ...
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        # 将collision_detected初始化为与环境数量相同的张量
        # 初始化碰撞检测参数
    ...
    def check_termination(self):
        ...
        #     self.reset_buf |= collision_detected
            ...
            #     print("\n=== 碰撞终止信息 ===")
            #     print(f"发生碰撞的环境数量: {torch.sum(collision_detected).item()}")
            #     print(f"碰撞环境索引: {torch.where(collision_detected)[0].cpu().numpy()}")
            #     print(f"最大穿透深度: {torch.max(penetration_depths).item():.4f} m")
            #     print("===================\n")
    ...
    def _post_physics_step_callback(self):
        ...
        if getattr(self.cfg.commands, "is_avoid_obstacles", False):
            self._update_command_by_obstacles()

        # 检测碰撞
        if hasattr(self, 'volume_sample_points'):
            self.refresh_volume_sample_points()  # 确保这行代码存在
            # print("volume_sample_points", self.volume_sample_points)
            penetration_depths = self.terrain.get_penetration_depths(self.volume_sample_points.view(-1, 3)).view(self.num_envs, -1)
            penetration_depths *= torch.norm(self.volume_sample_points_vel, dim= -1) + 1e-3

            # 更新碰撞状态
            self.collision_detected = torch.any(penetration_depths > 0.01, dim=1)  # 使用1cm作为阈值
            
            # # 打印调试信息
            # if torch.any(self.collision_detected):
            #     print("\n=== 碰撞检测信息 ===")
            #     print(f"发生碰撞的环境数量: {torch.sum(self.collision_detected).item()}")
            #     print(f"碰撞环境索引: {torch.where(penetration_depths > 0.01)[0].cpu().numpy()}")  # 修正这里
            #     print(f"最大穿透深度: {torch.max(penetration_depths).item():.4f} m")
            #     print("===================\n")
    ...
    def _update_command_by_obstacles(self): 
        """
    避障函数：根据障碍物检测结果调整控制指令。
    :param env_ids: 需要更新控制指令的环境 ID 列表

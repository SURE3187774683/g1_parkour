...
from legged_gym.envs.tron1.tron1_field_config import Tron1FieldCfg, Tron1FieldCfgPPO, Tron1RoughCfgPPO

multi_process_ = False
class Tron1DistillCfg( Tron1FieldCfg ):
    

    class env( Tron1FieldCfg.env ):
        num_envs = 256
        obs_components = [
            "lin_vel",
            "ang_vel",
    ...
    class terrain( Tron1FieldCfg.terrain ):
        if multi_process_:
            num_rows = 4
            num_cols = 1
        else:
        ...
        BarrierTrack_kwargs = merge_dict(Tron1FieldCfg.terrain.BarrierTrack_kwargs, dict(
            leap= dict(
                length= [0.05, 0.8],
                depth= [0.5, 0.8],
    ...
    class sensor( Tron1FieldCfg.sensor ):
    ...
    class commands( Tron1FieldCfg.commands ):
        ...
        lin_cmd_cutoff = 0.2
        class ranges( Tron1FieldCfg.commands.ranges ):
            # lin_vel_x = [0.6, 1.8]
            lin_vel_x = [-0.6, 2.0]
    ...
    class normalization( Tron1FieldCfg.normalization ):
        class obs_scales( Tron1FieldCfg.normalization.obs_scales ):
            forward_depth = 1.0

    class noise( Tron1FieldCfg.noise ):
        add_noise = False
        class noise_scales( Tron1FieldCfg.noise.noise_scales ):
            forward_depth = 0.0
    ...
    class curriculum( Tron1FieldCfg.curriculum ):
        no_moveup_when_fall = False

    class sim( Tron1FieldCfg.sim ):
        no_camera = False
    
logs_root = osp.join(osp.dirname(osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))), "logs")
class Tron1DistillCfgPPO( Tron1FieldCfgPPO ):
    class algorithm( Tron1FieldCfgPPO.algorithm ):
        entropy_coef = 0.0
        using_ppo = False
        ...
        teacher_ac_path = osp.join(logs_root, "field_Tron1",
            "Apr11_17-14-24_Based_plane6000_Adjust_body_measure_0feet_regulation",
            "model_73000.pt"
        )

        # class teacher_policy( Tron1FieldCfgPPO.policy ):
        ...
        class teacher_policy( Tron1FieldCfgPPO.policy ):
            num_actor_obs = 36 + 21 * 11
            num_critic_obs = 36 + 21 * 11
            num_actions = 8
            obs_segments = OrderedDict([
                ("lin_vel", (3,)),
                ("ang_vel", (3,)),
                ...
                ("dof_pos", (8,)),
                ("dof_vel", (8,)),
                ("last_actions", (8,)), # till here: 3+3+3+3+8+8+8 = 36
                ("height_measurements", (1, 21, 11)),
            ])

    class policy( Tron1RoughCfgPPO.policy ):
    ...
    class runner( Tron1FieldCfgPPO.runner ):
        ...
        resume = True
        load_run = osp.join(logs_root, "field_Tron1",
            "Apr11_17-14-24_Based_plane6000_Adjust_body_measure_0feet_regulation",
        )
        ckpt_manipulator = "replace_encoder0" if "field_Tron1" in load_run else None

        run_name = "".join(["Tron1_",
            ("{:d}skills".format(len(Tron1DistillCfg.terrain.BarrierTrack_kwargs["options"]))),

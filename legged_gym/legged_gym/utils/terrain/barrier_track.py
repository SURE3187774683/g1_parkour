...
class BarrierTrack:
    ...
    block_info_dim = 4 # height, depth, width_range, position_range
    ...
    def get_hurdle_track(self,
        ...
        # 根据难度动态调整障碍物宽度
        if isinstance(self.track_kwargs["hurdle"]["width_range"], (tuple, list)):
            hurdle_width = (1 - difficulty) * self.track_kwargs["hurdle"]["width_range"][0] + difficulty * self.track_kwargs["hurdle"]["width_range"][1]
        else:
            hurdle_width = self.track_kwargs["hurdle"]["width_range"]
        
        # 根据难度动态调整障碍物位置
        if isinstance(self.track_kwargs["hurdle"]["position_range"], (tuple, list)):
            hurdle_position = (1 - difficulty) * self.track_kwargs["hurdle"]["position_range"][0] + difficulty * self.track_kwargs["hurdle"]["position_range"][1]
        else:
            hurdle_position = self.track_kwargs["hurdle"]["position_range"]
        
        depth_px = int(hurdle_depth / self.cfg.horizontal_scale)
        height_value = hurdle_height / self.cfg.vertical_scale
        wall_thickness_px = int(wall_thickness / self.cfg.horizontal_scale) + 1
        track_width_px = int(self.track_kwargs["track_width"] / self.cfg.horizontal_scale)
        
        # 计算障碍物的宽度和位置
        center_y_px = int(track_width_px / 2)
        hurdle_start_y_px = center_y_px + int(hurdle_position / self.cfg.horizontal_scale) - int(hurdle_width / self.cfg.horizontal_scale / 2)
        hurdle_end_y_px = hurdle_start_y_px + int(hurdle_width / self.cfg.horizontal_scale)
        ...
        if not virtual:
            track_heightfield[
                1:depth_px + 1,
                max(wall_thickness_px, hurdle_start_y_px):min(track_width_px - wall_thickness_px, hurdle_end_y_px),
            ] += height_value
        ...
        block_info = torch.tensor([
            hurdle_depth,
            hurdle_height_,
            hurdle_width,  # 添加障碍物宽度
            hurdle_position,  # 添加障碍物位置
        ], dtype=torch.float32, device=self.device)
    ...
    #         block_infos[:, 2] < 0.,
    #     )
    #     hurdle_over_mask = torch.logical_and(
    #         positions_in_block[:, 2] > block_infos[:, 2],
    #         positions_in_block[:, 2] > 0, # to avoid the penetration of virtual obstacle in jump down.
    #     ) # (n_points,)
    #     if (block_infos[:, 2] < 0.).any():
    ...
    #     penetrated_mask = torch.logical_and(
    #         torch.logical_or(in_up_mask, in_down_mask),
    #         (torch.logical_not(hurdle_over_mask)),
    #     )
    #     if mask_only:
    #         return penetrated_mask.to(torch.float32)
    ...
    def get_hurdle_penetration_depths(self,
        ...
        """
        Compute the horizontal penetration depths for hurdle obstacles as the inverse of the horizontal distance to the center of the hurdle.
        :param block_infos: torch tensor of shape (N, 4) containing [depth, height, width, position] for each block.
        :param positions_in_block: torch tensor of shape (N, 3) containing the positions of the sample points in the block frame.
        :param mask_only: If True, return a binary mask indicating penetration (1) or no penetration (0).
        :return: torch tensor of shape (N,) containing the horizontal penetration depths.
        """
        # Extract information from block_infos
        hurdle_depth = block_infos[:, 1]  # Depth of the hurdle
        hurdle_height = block_infos[:, 2]  # Height of the hurdle
        hurdle_width = block_infos[:, 3]  # Width of the hurdle
        hurdle_position = block_infos[:, 4]  # Position of the hurdle (offset from the center)

        # Extract positions in block frame
        x = positions_in_block[:, 0]  # Forward distance
        y = positions_in_block[:, 1]  # Lateral distance
        z = positions_in_block[:, 2]  # Vertical distance

        # Calculate the penetration mask
        in_hurdle_mask = (
            (x < hurdle_depth) &
            (y > hurdle_position - hurdle_width / 2) &
            (y < hurdle_position + hurdle_width / 2) &
            (z < hurdle_height)
        )

        if mask_only:
            return in_hurdle_mask.to(torch.float32)

        # Calculate the horizontal penetration depths
        penetration_depths = torch.zeros_like(x)

        # Calculate the horizontal distance to the center of the hurdle
        distance_to_center = torch.sqrt((x - hurdle_depth / 2) ** 2 + (y - hurdle_position) ** 2)

        # Avoid division by zero by adding a small epsilon
        epsilon = 1e-6
        distance_to_center = torch.clamp(distance_to_center, min=epsilon)

        # Calculate the inverse of the distance to the center
        inverse_distance_to_center = 1.0 / distance_to_center

        # Assign the inverse distance to the penetration depths buffer for points inside the hurdle
        penetration_depths[in_hurdle_mask] = inverse_distance_to_center[in_hurdle_mask]
    ...
    def draw_virtual_hurdle_track(self,
        ...
        """
    绘制虚拟的 hurdle 障碍物。
    :param block_info: 障碍物信息，包含 depth, height, width, position
    :param block_origin: 障碍物的起始位置
    """
        # 从 block_info 中提取障碍物的深度、高度、宽度和位置
        hurdle_depth = block_info[1]  # 障碍物的深度
        hurdle_height = block_info[2]  # 障碍物的高度
        hurdle_width = block_info[3]  # 障碍物的宽度
        hurdle_position = block_info[4]  # 障碍物的位置（相对于道路中心的偏移量）

        # 计算障碍物的中心位置
        center_y = block_origin[1] + hurdle_position  # 障碍物中心的 y 坐标

        # 创建一个线框长方体几何对象来表示障碍物
        geom = gymutil.WireframeBoxGeometry(
            hurdle_depth,  # 障碍物的深度
            hurdle_width,  # 障碍物的宽度
            hurdle_height,  # 障碍物的高度
            pose=None,
            color=(0, 0, 1),  # 障碍物的颜色（蓝色）
        )

        # 设置障碍物的位置
        pose = gymapi.Transform(
            gymapi.Vec3(
                block_origin[0] + hurdle_depth / 2,  # 障碍物中心的 x 坐标
                center_y,  # 障碍物中心的 y 坐标
                block_origin[2] + hurdle_height / 2,  # 障碍物中心的 z 坐标
            ),
            r=None,
        )

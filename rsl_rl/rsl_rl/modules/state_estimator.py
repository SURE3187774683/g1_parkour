...
class EstimatorMixin:
    ...
    def act(self, observations, masks=None, hidden_states=None, inference= False):
        if torch.isnan(observations).any() or torch.isinf(observations).any():
            raise ValueError("Invalid values in observations")
        ...
        if self.is_recurrent and self.use_actor_rnn:
            ...
            if torch.isnan(input_s).any() or torch.isinf(input_s).any():
                raise ValueError("Invalid values in input_s")
